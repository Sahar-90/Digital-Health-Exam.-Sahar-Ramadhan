...
ui <- fluidPage(
  titlePanel("Medical Insurance Charges Visualization"),
  sidebarLayout(
    sidebarPanel(
      selectInput("xvar", "X-axis variable", choices = c("age", "sex", "bmi")),
      selectInput("yvar", "Y-axis variable", choices = c("charges")),
      radioButtons("sex", "Sex", choices = c("Both", "Male", "Female"), selected = "Both"),
      sliderInput("ageRange", "Age Range:", min = 18, max = 64, value = c(18, 64)),
      sliderInput("bmiRange", "BMI Range:", min = 15, max = 55, value = c(15, 55))
    ),
    mainPanel(
      plotOutput("plot")
    )
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ...
    filtered_data <- data
    if (input$sex != "Both") {
      filtered_data <- filtered_data[filtered_data$sex == tolower(input$sex), ]
    }
    filtered_data <- filtered_data[filtered_data$age >= input$ageRange[1] & filtered_data$age <= input$ageRange[2], ]
    filtered_data <- filtered_data[filtered_data$bmi >= input$bmiRange[1] & filtered_data$bmi <= input$bmiRange[2], ]
    
    # Create the plot using tidy evaluation with aes()
    ggplot(filtered_data, aes(x = !!sym(input$xvar), y = !!sym(input$yvar))) +
      geom_point() +
      labs(x = input$xvar, y = input$yvar, title = paste("Scatterplot of", input$yvar, "vs", input$xvar)) +
      theme_minimal()
  })
}
